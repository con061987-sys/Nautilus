...
class MetaScheduleAdapter:
    ...
    def tune(
        ...
        Returns:
            MappedTuningConfig with optimal parameters.
        """
        if not TVM_AVAILABLE:
            logger.info("TVM unavailable, returning default config")
        ...
        try:
            ...
            logger.error("MetaSchedule tuning failed: %s", exc, exc_info=True)
            return MappedTuningConfig.defaults()
    ...
    def _run_tune_with_timeout(
        ...
        """Run tune_tir with a wall-clock timeout.

        Uses a file-based timeout approach (cheaper than subprocess).
        """
